...
export default function User({ data }) {
  ...
  const activityMessage = data.spotify ? `Listening to ${data.spotify.song} by ${data.spotify.artist}` : data.activities[0] ? `Playing ${data.activities[0].name}` : "Doing nothing"

  return (
      <div className="m-5">
        <Tooltip className="tooltip-bottom" message={activityMessage}>
          <Avatar className={`mb-5 rounded-full ring ${statusColor} ring-offset-base-100 ring-offset-2`} src={avatarUrl} size="lg" shape="circle"/>
        </Tooltip>
